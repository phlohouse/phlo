...
async def test_preview_cancels_latest_continuation_after_row_limit(monkeypatch) -> None:
    ...
    async def respond(request: httpx.Request) -> httpx.Response:
        requests.append(
            (request.method, request.url.path, request.headers.get("x-trino-session", ""))
        )
    ...
    async def respond(request: httpx.Request) -> httpx.Response:
        ...
        if request.method == "POST":
            return httpx.Response(
                ...
                json={
                    "columns": [{"name": "id", "type": "bigint"}],
                    "nextUri": "http://trino:8080/v1/next/2",
                },
            )
        ...
        if request.method == "POST":
            return httpx.Response(
                ...
                json={
                    ...
                    "nextUri": "http://trino:8080/v1/next/3",
                },
            )
        if request.method == "DELETE":
            return httpx.Response(204)
...
async def test_preview_cancels_query_when_client_disconnects(monkeypatch) -> None:
    ...
    async def respond(request: httpx.Request) -> httpx.Response:
        ...
        if request.method == "POST":
            return httpx.Response(
                200,
                json={"columns": [], "nextUri": "http://trino:8080/v1/next/4"},
            )
        if request.method == "DELETE":
            return httpx.Response(204)
