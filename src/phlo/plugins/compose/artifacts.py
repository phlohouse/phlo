...
def write_compose_layers(
    ...
    *,
    ...
    if not marker.exists() or SHARED_LAYOUT_MARKER not in marker.read_text(encoding="utf-8"):
    ...
    if changes:
        ...
        host_path.write_text(header + yaml.safe_dump(changes, sort_keys=False), encoding="utf-8")
    else:
        host_path.unlink(missing_ok=True)
