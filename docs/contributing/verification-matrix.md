...
| One Python package | `uv run --locked --package <package> --with-editable ./packages/phlo-testing --with pytest pytest -m "not integration" --tb=short packages/<package>/tests` | Test on Python 3.12; `make check`; build the package when metadata or package data changes |
