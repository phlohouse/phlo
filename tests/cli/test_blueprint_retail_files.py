...
def test_resource_digest_ignores_python_bytecode(
    tmp_path: Path, blueprint_contract: ModuleType, monkeypatch: pytest.MonkeyPatch
) -> None:
    resources = tmp_path / "resources"
    (resources / "template.py").parent.mkdir(parents=True)
    (resources / "template.py").write_text("value = 1\n", encoding="utf-8")
    monkeypatch.setattr(blueprint_contract, "RESOURCES_DIR", resources)
    expected = blueprint_contract.resource_digest()

    cache = resources / "__pycache__"
    cache.mkdir()
    (cache / "template.cpython-312.pyc").write_bytes(b"generated bytecode")

    assert blueprint_contract.resource_digest() == expected
